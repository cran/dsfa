...
#'
#' @aliases dcomperr, pcomperr, qcomperr, rcomperr
#'
...
#' @param dist \eqn{normhnorm} for normal-halfnormal and \eqn{normexp} for normal-exponential distribution.
...
#'
#' @examples
#' pdf <- dcomperr(x=seq(-3, 3, by=0.1), mu=1, sigma_v=2, par_u=3, s=-1, dist="normhnorm")
#' cdf <- pcomperr(q=seq(-3, 3, by=0.1), mu=1, sigma_v=2, par_u=3, s=-1, dist="normhnorm")
#' q <- qcomperr(p=seq(0.1, 0.9, by=0.1), mu=1, sigma_v=2, par_u=3, s=-1, dist="normhnorm")
#' r <- rcomperr(n=100, mu=1, sigma_v=2, par_u=3, s=-1, dist="normhnorm")
#'
#' @references
#' \itemize{
...
#' @export
#dcomperr
dcomperr<-function(x=0, mu=0, sigma_v=1, par_u=1,  s=-1, dist=NULL, deriv = 0,  xg=NULL, log.p=FALSE){
  #Check for correct inputs
  if (dist!="normexp"&dist!="normhnorm"){
    stop(paste("Wrong input for dist", "\n", ""))
  }
  ...
  if(dist=="normexp"){
    out<-dnormexp(x=x, mu=mu, sigma_v=sigma_v, lambda=par_u, s=s, deriv = deriv,  xg=xg, log.p = log.p)
  }

  if(dist=="normhnorm"){
    out<-dnormhnorm(x=x, mu=mu, sigma_v=sigma_v, sigma_u=par_u, s=s, deriv = deriv,  xg=xg, log.p = log.p)
  }
...
#' @export
pcomperr<-function(q, mu=0, sigma_v=1, par_u=1,  s=-1, dist=NULL, deriv = 0,  xg=NULL, lower.tail = TRUE, log.p=FALSE){
  ...
  if(dist=="normexp"){
    out<-pnormexp(q=q, mu=mu, sigma_v=sigma_v, lambda=par_u, s=s, deriv = deriv,  xg=xg, lower.tail = lower.tail, log.p = log.p)
  }

  if(dist=="normhnorm"){
    out<-pnormhnorm(q=q, mu=mu, sigma_v=sigma_v, sigma_u=par_u, s=s, deriv = deriv,  xg=xg, lower.tail = lower.tail, log.p = log.p)
  }
...
#' @export
qcomperr<-function(p, mu=0, sigma_v=1, par_u=1,  s=-1, dist=NULL, lower.tail = TRUE, log.p=FALSE){
  ...
  if(dist=="normexp"){
    out<-qnormexp(p=p, mu=mu, sigma_v=sigma_v, lambda=par_u, s=s, lower.tail = lower.tail, log.p = log.p)
  }

  if(dist=="normhnorm"){
    out<-qnormhnorm(p=p, mu=mu, sigma_v=sigma_v, sigma_u=par_u, s=s, lower.tail = lower.tail, log.p = log.p)
  }
...
#' @export
rcomperr<-function(n, mu=0, sigma_v=1, par_u=1,  s=-1, dist=NULL){
  ...
  if(dist=="normexp"){
    out<-rnormexp(n=n, mu=mu, sigma_v=sigma_v, lambda=par_u, s=s)
  }

  if(dist=="normhnorm"){
    out<-rnormhnorm(n=n, mu=mu, sigma_v=sigma_v, sigma_u=par_u, s=s)
  }
