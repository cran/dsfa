...
#' @export
#dcomperr
dcomperr<-function(x=0, mu=0, sigma_v=1, par_u=1,  s=-1, family="normhnorm", deriv = 0,  tri=NULL, log.p=FALSE){
  ...
  if(family=="normexp"){
    out<-dnormexp(x=x, mu=mu, sigma_v=sigma_v, lambda=par_u, s=s, deriv = deriv,  tri=tri, log.p = log.p)
  }

  if(family=="normhnorm"){
    out<-dnormhnorm(x=x, mu=mu, sigma_v=sigma_v, sigma_u=par_u, s=s, deriv = deriv,  tri=tri, log.p = log.p)
  }
...
#' @export
pcomperr<-function(q, mu=0, sigma_v=1, par_u=1,  s=-1, family="normhnorm", deriv = 0,  tri=NULL, lower.tail = TRUE, log.p=FALSE){
  ...
  if(family=="normexp"){
    out<-pnormexp(q=q, mu=mu, sigma_v=sigma_v, lambda=par_u, s=s, deriv = deriv,  tri=tri, lower.tail = lower.tail, log.p = log.p)
  }

  if(family=="normhnorm"){
    out<-pnormhnorm(q=q, mu=mu, sigma_v=sigma_v, sigma_u=par_u, s=s, deriv = deriv,  tri=tri, lower.tail = lower.tail, log.p = log.p)
  }
...
#' @export
qcomperr<-function(p, mu=0, sigma_v=1, par_u=1,  s=-1, family="normhnorm", lower.tail = TRUE, log.p=FALSE){
...
#' @export
rcomperr<-function(n, mu=0, sigma_v=1, par_u=1,  s=-1, family="normhnorm"){
