...
#'
#'
#' @param link seven item list specifying the link for the \eqn{\mu_1}, \eqn{\sigma_{V1}}, \eqn{\sigma_{U1}}, \eqn{\lambda_1}, \eqn{\mu_2}, \eqn{\sigma_{V2}}, \eqn{\sigma_{U2}}, \eqn{\lambda_2} and \eqn{\tau} parameters. See details.
...
#' @param dist vector of length three, specifying the bivariate distribution.  First element is the name of the first marginal distribution. Second element is the name of the second marginal distribution. Third element specifies the copula. See \code{\link{dcop}} for more details.
#'
...
#' }
#' @export
comperr_mv <- function(link = list("identity", "log", "log", "identity", "log", "log", "tanh"), s1 = -1, s2 = -1, dist=c("normhnorm","normhnorm",1)){
  ...
  okLinks <- list("identity", "log", "log","identity", "log", "log", "tanh")
  stats <- list()
  param.names <- c("mu1", "sigma_v1", "par_u1","mu2", "sigma_v2", "par_u2","Tau")
  for (i in 1:6) { # Links for "mu1", "sigma_v1", "sigma_u1","mu2", "sigma_v2", "sigma_u2","Tau"
  ...
  if (link[[7]] %in% okLinks[[7]]) {
    ...
    stats[[7]]$linkfun <- eval(parse(text = paste("function(mu) log((mu+1)/(1-mu))")))
    stats[[7]]$linkinv <- eval(parse(text = paste("function(eta) (exp(eta)-1)/(exp(eta)+1)")))
    stats[[7]]$mu.eta <- eval(parse(text = paste("function(eta) 1/(1 + cosh(eta))")))
    stats[[7]]$d2link <- eval(parse(text = paste("function(mu) 4*mu/(mu^2-1)^2"))) #-((2 * exp(eta) * (-1 + exp(eta)))/(1 + exp(eta))^3)
    stats[[7]]$d3link <- eval(parse(text = paste("function(mu) -4*(3*mu^2+1)/(mu^2-1)^3"))) #(2 * exp(eta) * (1 - 4 * exp(eta) + exp(2 * eta)))/(1 + exp(eta))^4
    stats[[7]]$d4link <- eval(parse(text = paste("function(mu) 48*(mu^3+mu)/(mu^2-1)^4"))) #-((2 * exp(eta)*  (-1 + 11 * exp(eta) - 11 * exp(2 * eta) + exp(3 * eta)))/(1 + exp(eta))^5)
  }
  ...
  residuals <- function(object, type = c("deviance", "response")) {
    ...
    para1<-reparametrize(mu = object$fitted[,1], sigma_v = object$fitted[,2], par_u = object$fitted[,3],
                         s = object$family$s1, dist = object$family$dist[1])
    para2<-reparametrize(mu = object$fitted[,4], sigma_v = object$fitted[,5], par_u = object$fitted[,6],
                         s = object$family$s2, dist = object$family$dist[2])
  ...
  ll <- function(y, X, coef, wt, family, offset = NULL, deriv=0, d1b=0, d2b=0, Hp=NULL, rank=0, fh=NULL, D=NULL) {
    ...
    s2       <- family$s2
    Tau      <- family$linfo[[7]]$linkinv( eta6 )

    ##Define parameters
    l0<-dcomperr_mv(x1=y[,1], mu1=mu1, sigma_v1=sigma_v1, par_u1=par_u1, s1=s1, dist1 = family$dist[1],
                    x2=y[,2], mu2=mu2, sigma_v2=sigma_v2, par_u2=par_u2, s2=s2, dist2 = family$dist[2],
                     Tau=Tau, family=family$dist[3], deriv=2, xg=family$tri, log.p = TRUE)
  ...
  initialize <- expression({
    ...
    if (is.null(start)) {
      ...
      u1<-pcomperr(q=y[,1], mu=m1$fitted.values[,1], sigma_v=m1$fitted.values[,2], par_u=m1$fitted.values[,3], s=family$s1, dist=family$dist[1], log.p=FALSE)
      u2<-pcomperr(q=y[,2], mu=m2$fitted.values[,1], sigma_v=m2$fitted.values[,2], par_u=m2$fitted.values[,3], s=family$s2, dist=family$dist[2], log.p=FALSE)

      family$data$y3<-rep(1,nrow(Y))
      adj_par_formula<-update.formula(family$formula[[7]], y3 ~ .)
      m3<-mgcv::gam(list(adj_par_formula),
                    data=family$data, family=cop(u1=u1, u2=u2, cop_family=family$dist[3]), optimizer = c("efs"))
  ...
  rd <- function(mu, wt, scale, family) {
    ...
    return(rcomperr_mv(ncol(mu), mu1 = mu[,1], sigma_v1 = mu[,2], par_u1 = mu[,3], s1=family$s1, dist1=family$dist[1],
                                 mu2 = mu[,4], sigma_v2 = mu[,5], par_u2 = mu[,6], s2=family$s2, dist2=family$dist[2],
                                 Tau= mu[,7], family=family$dist[3]))
  } ## random number generation
  ...
  cdf <- function(q, mu, wt, scale, logp, family) {
    ...
    return(pcomperr_mv(q1=q[,1], mu1 = mu[,1], sigma_v1 = mu[,2], par_u1 = mu[,3], s1=family$s1, dist1=family$dist[1],
                       q2=q[,2], mu2 = mu[,4], sigma_v2 = mu[,5], par_u2 = mu[,6], s2=family$s2, dist2=family$dist[2],
                       Tau=mu[,7], family=family$dist[3],
                       log.p = logp))
  } ##cumulative distribution function
  ...
  predict <- function(family,se=FALSE,eta=NULL,y=NULL,X=NULL,
    ...
    para1<-reparametrize(mu = mu1, sigma_v = sigma_v1, par_u = par_u1, s=family$s1, dist=family$dist[1])
    para2<-reparametrize(mu = mu2, sigma_v = sigma_v2, par_u = par_u2, s=family$s2, dist=family$dist[2])
