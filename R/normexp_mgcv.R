...
normexp <- function(link = list("identity", "log", "log"), s = -1){
  ...
  ll <- function(y, X, coef, wt, family, offset = NULL, deriv=0, d1b=0, d2b=0, Hp=NULL, rank=0, fh=NULL, D=NULL) {
    ...
    l0<-dnormexp(x=y, mu=mu, sigma_v=sigma_v, lambda=lambda, s=s, deriv=4, xg=family$tri, log.p = TRUE)
  ...
  initialize <- expression({
    ...
    if (is.null(start)) {
      ...
      para<-reparametrize(mean = qr.fitted(qr.x_all, y),
                            ...
                            skew = rep((sum((y - mean(y))^3)/length(y))/(sum((y - mean(y))^2)/length(y))^(3/2),length(y)), s=s, dist="normexp")
