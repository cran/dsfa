...
#'
#normhnorm
dnormhnorm <- function(x, mu=0, sigma_v=1, sigma_u=1, s=-1, deriv=0, tri=NULL, log.p = FALSE, check=TRUE){
  #Density function of the normhnorm distribution

  #Check inputs arguments
  if(check){
    arguments<-check_arguments(x=x, mu=mu, sigma_v=sigma_v, par_u=sigma_u, s=s, family="normhnorm", deriv=deriv)
    if(arguments$recycle){
      x<-arguments$x
      mu<-arguments$mu
      sigma_v<-arguments$sigma_v
      sigma_u<-arguments$par_u
    }
  }
  ...
  z<-(x-mu)*tau
  az<-a*z
  zeta_az<-zeta(az, deriv=deriv)

  #Get loglikelihood
  out  <-  log(1/sqrt(2*pi))+log(tau)-0.5*z^2+zeta_az
  ...
  if(deriv>0){
    ...
    attr(az,"gradient")<-cbind(d1azdmu1, d1azdv1, d1azdu1)
    attr(az,"hessian")<-cbind(d2azdmu2, d2azdmu1dv1, d2azdmu1du1, d2azdv2, d2azdv1du1, d2azdu2)
    ...
    if(deriv>2){
      ...
      attr(az,"l3")<-cbind(d3azdmu3,d3azdmu2dv1,d3azdmu2du1,d3azdmu1dv2,d3azdmu1dv1du1,d3azdmu1du2,d3azdv3,d3azdv2du1,d3azdv1du2,d3azdu3)
      attr(az,"l4")<-cbind(d4azdmu4,d4azdmu3dv1,d4azdmu3du1,
    ...
    l0<-chainrule(f=zeta_az, g=az, deriv=deriv, tri=tri)
...
#' @export
pnormhnorm <- function(q, mu=0, sigma_v=1, sigma_u=1, s=-1, deriv=0, tri=NULL, lower.tail = TRUE, log.p = FALSE, check=TRUE){
  #Distribution function of the normhnorm distribution

  #Check inputs arguments
  if(check){
    arguments<-check_arguments(q=q, mu=mu, sigma_v=sigma_v, par_u=sigma_u, s=s, family="normhnorm", deriv=deriv)
    if(arguments$recycle){
      q<-arguments$x
      mu<-arguments$mu
      sigma_v<-arguments$sigma_v
  ...
  pnorm_z<-pnorm(z, deriv=deriv)
  out <-pnorm_z-2*OwenT(z, a)
  ...
  if(deriv>0){
    ...
    # attr(pnorm_z,"gradient")<-d1pnormdz1
    # attr(pnorm_z,"hessian")<-d2pnormdz2

    attr(z,"gradient")<-cbind(d1zdmu1, d1zdv1, d1zdu1)
    attr(z,"hessian")<-cbind(d2zdmu2, d2zdmu1dv1, d2zdmu1du1, d2zdv2, d2zdv1du1, d2zdu2)

    l0<-chainrule(f=pnorm_z,  g=z,  deriv=deriv, tri=tri)
    # l0<-chainrule(f1=d1pnormdz1, f2=d2pnormdz2, f3=NULL, f4=NULL,
...
#' @export
qnormhnorm <- function(p, mu=0, sigma_v=1, sigma_u=1, s=-1, lower.tail = TRUE, log.p = FALSE, check=TRUE){
  #Quantile function of the normhnorm distribution

  #Check inputs arguments
  if(check){
    arguments<-check_arguments(p=p, mu=mu, sigma_v=sigma_v, par_u=sigma_u, s=s, family="normhnorm", deriv=0)
    if(arguments$recycle){
      p<-arguments$x
      mu<-arguments$mu
      sigma_v<-arguments$sigma_v
  ...
  # out<-c()
  # for(n in 1:N){
  #   out[n]<-sn::qsn(p=p[i], xi=para$xi[i], omega=para$omega[i], alpha=para$alpha[i])
  # }
...
#' @export
rnormhnorm <- function(n, mu=0, sigma_v=1, sigma_u=1, s=-1, check=TRUE){
  #Function to generate n random numbers from the normhnorm distribution

  #Check inputs arguments
  if(check){
    arguments<-check_arguments(n=n, mu=mu, sigma_v=sigma_v, par_u=sigma_u, s=s, family="normhnorm", deriv=0)
    if(arguments$recycle){
      n<-arguments$x
      mu<-arguments$mu
      sigma_v<-arguments$sigma_v
